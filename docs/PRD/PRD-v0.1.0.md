...
Comparison UI v0 先提供“新建比对”：左侧“基准版本”、右侧“当前版本”，每侧选择或拖入一个 DOCX；不要求先建立项目、模板或轮次。该入口不取代第 11 章后续项目批量导入，且复用正式 Comparison Engine、独立历史记录和原始路径/hash 规则。完成范围及验收以 `docs/tasks/comparison-ui-v0.md` 为准，不代表完整项目管理已实现。
