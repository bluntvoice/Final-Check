...
> 历史阶段规范说明（2026-09-14）：本文保留原 Phase、DONE 和验收记录。后续以 [校准 PRD](../PRD/PRD-v0.1.0.md) 为唯一需求基准；原“先建项目/逐文件必填角色轮次/默认按轮次/对方优先 Own/Quick Compare 默认不项目化”描述属于该阶段历史范围，不约束新 Stage A。新目标为成功后自动项目化、自动 V1/V2、轮次高级可选、普通比较角色可暂未指定、绑定模板当前版本直接默认；仍保留 Own-only baseline、轮次模型和冻结历史。兼容方案及新验收须在后续任务验证，本次不改代码/schema，不重新执行已完成任务或生成安装包。
