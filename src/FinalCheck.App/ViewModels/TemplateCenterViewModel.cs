...
{
    // A list refresh can temporarily clear its selection without starting a new-template draft.
    ...
    partial void OnSelectedTemplateChanged(Template? value)
    {
        ...
        if (editingTemplateId != value.TemplateId) { SelectedVersion = null; DeletePrompt = false; PreviewBlocks = []; }
    ...
    public async Task SelectTemplateAsync(Template? selected)
    {
        ...
        {
            var detail = await service.GetAsync(selected.TemplateId); SelectedTemplate = detail.Template; editingTemplateId = detail.Template.TemplateId;
    ...
    public Task AcceptFilesAsync(IEnumerable<string> paths)
    {
        ...
        else { SourcePath = items[0]; if (editingTemplateId is null && string.IsNullOrWhiteSpace(Name)) Name = Path.GetFileNameWithoutExtension(items[0]); Message = "请核对模板名称与版本后导入。"; }
        return Task.CompletedTask;
    }
    [RelayCommand] private void NewTemplate()
    { if (IsBusy) return; editingTemplateId = null; SelectedTemplate = null; SelectedVersion = null; DeletePrompt = false; PreviewBlocks = []; Versions.Clear(); Name = ""; ContractType = ""; Notes = ""; SourcePath = ""; Version = "1.0.0"; Enabled = true; Message = "选择或拖入 DOCX，填写新模板信息。"; }
    ...
    private async Task RefreshCoreAsync()
    {
        ...
        SelectedTemplate = rows.FirstOrDefault(item => item.TemplateId == editingTemplateId);
    ...
    [RelayCommand] private Task ImportAsync() => PerformAsync(async () =>
    {
        ...
        var detail = await service.ImportAsync(editingTemplateId, Name, ContractType, Version, SourcePath);
    ...
    { if (service is null || editingTemplateId is not { } id) return; await service.UpdateAsync(id, Name, ContractType, Notes, Enabled); await RefreshCoreAsync(); Message = "模板信息已保存。"; });
    [RelayCommand] private Task SetCurrentAsync() => PerformAsync(async () =>
    {
        if (service is null || editingTemplateId is not { } id || SelectedVersion is null) return;
        await service.SetCurrentAsync(id, SelectedVersion.TemplateVersionId);
        var detail = await service.GetAsync(id); Versions.Clear(); foreach (var v in detail.Versions) Versions.Add(v); SelectedVersion = detail.Current; Message = "当前版本已切换，历史版本与比对保持不变。";
    });
    ...
    {
        if (service is null || editingTemplateId is not { } id) return;
        var references = await service.ReferencesAsync(id);
    ...
    { if (service is null || editingTemplateId is not { } id || !DeletePrompt) return; await service.DeleteAsync(id, true); DeletePrompt = false; editingTemplateId = null; SelectedTemplate = null; SelectedVersion = null; Versions.Clear(); await RefreshCoreAsync(); Message = "模板已移出模板中心；历史与原始文件保留。"; });
