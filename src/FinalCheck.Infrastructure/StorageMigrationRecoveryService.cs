    ...
    IStorageDatabaseMigrationService databases, StorageMigrationJournal journal)
{
    public async Task<IReadOnlyList<StorageRecoveryDiagnostic>> RecoverAsync(CancellationToken cancellationToken = default)
    {
        ...
        var pending = journal.Load().Where(record => record.Status is StorageMigrationStatus.InProgress or StorageMigrationStatus.NeedsReview).ToArray();
        ...
        try
        {
            ...
            foreach (var record in pending)
            {
                ...
                else if (locator.Current == record.Source)
                {
                    ...
                    // Close this interrupted attempt without deleting its files. Otherwise a later successful retry
                    // would make the obsolete InProgress receipt disagree with the new locator on every startup.
                    journal.Save(record with { Status = StorageMigrationStatus.Failed, FailureCode = "InterruptedBeforeCommitOldDataRetained", CompletedAt = DateTimeOffset.UtcNow });
                }
                else
                {
