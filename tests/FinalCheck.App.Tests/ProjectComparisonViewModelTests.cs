...
public sealed class ProjectComparisonViewModelTests
{
    private sealed class Service(VersionManagementTests.Service versions) : IProjectComparisonService
    {
        ...
        public Task<ComparisonWorkflowResult> CompareAsync(ProjectComparisonSelection selection, IProgress<string>? progress = null, ComparisonIgnoreRules? ignoreRules = null, CancellationToken token = default) { Selection = selection; SavedRules = ignoreRules; LastType = selection.BaselineType; return Task.FromResult(ComparisonExecutionTests.Result()); }
    ...
    [Fact] public async Task RememberedTypePreselectsWithoutExecutingAndOwnSuggestionRemainsVisible()
    {
        ...
        vm.IgnorePunctuation = true; vm.IgnoreAllFormatting = true;
        ...
        Assert.True(service.SavedRules!.Punctuation); Assert.True(service.SavedRules.AllFormatting);
    }
